...
const OrderStatusAlert = ({ orderStatus, setOrderStatus }) => {
  ...
  const isPending = orderStatus.type === 'pending'
  ...
  const icon = isPending
    ? <AlertTriangle className="w-5 h-5 mr-2 mt-0.5 flex-shrink-0 text-yellow-600" />
    : orderStatus.success
  ...
  return (
    <div
      className={`fixed top-2 right-2 left-2 sm:top-4 sm:right-4 sm:left-4 sm:left-auto sm:max-w-md z-50 p-3 sm:p-4 rounded-lg shadow-lg ${containerClass}`}
    >
      <div className="flex items-start">
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* Pending: direct user to order history */}
          {isPending && (
            <button
              onClick={() => { setOrderStatus(null); navigate('/dashboard/orders') }}
              className="mt-2 text-xs font-semibold underline text-yellow-800 hover:text-yellow-900"
            >
              Check Order History →
            </button>
          )}
          ...
          {!orderStatus.success && !isPending && orderStatus.details && orderStatus.details.shortfall && (
